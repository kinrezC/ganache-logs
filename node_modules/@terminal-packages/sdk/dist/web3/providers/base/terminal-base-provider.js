...
class TerminalBaseProvider {
    constructor(_providerType, _isCustomProvider, _baseOptions) {
        ...
        this.web3Version = this._baseOptions.web3Version || enums_2.Web3Versions.two;
        this._baseProviderOptions = {
            ...
            networkSource: this._baseOptions.networkSource,
        };
     ...
     */
    saveJsonRpcLog(payload, result, responseTimeMs, isError = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._baseProviderOptions.source === source_types_1.SourceType.MetaMask) {
                ...
                if (this._provider && this._provider.enable) {
                    // get the MM accounts
                    const accounts = yield this._provider.enable();
                    if (accounts.length > 0) {
                        fromAddress = utils_1.Utils.toChecksumAddress(accounts[0]);
                    }
                }
            }
            // truffle expose the address here
            // so do others who copy truffle so get it if defined
            if (this._provider && this._provider.getAddress !== undefined) {
                const address = this._provider.getAddress();
                if (address) {
                    fromAddress = utils_1.Utils.toChecksumAddress(address);
                }
            }
            let headers = [];
            ...
            const logRequest = {
                ...
                fromAddress,
                networkSource: this._baseProviderOptions.networkSource,
            };
            yield this._apiService.saveJsonRpcLog(logRequest);
        });
    }
    /**
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (Array.isArray(response)) {
                    ...
                    else if ((this._baseProviderOptions.source ===
                        source_types_1.SourceType.TruffleHDWalletProvider ||
                        this._baseProviderOptions.source === source_types_1.SourceType.Truffle) &&
                        this._isCustomProvider) {
                        start = new Date().getTime();
                        result = yield this.sendAsyncOnProviderPromiseWrapper(payload);
                        // different providers pass back the whole payload not just the result
                        // hence the ugly result.result for this handle case.
                        response.result = result.result;
                    }
                    else if (this.web3Version === enums_2.Web3Versions.one &&
                        this._isCustomProvider) {
                        start = new Date().getTime();
                ...
                if (!errorMessage) {
                    ...
                    if (Array.isArray(response)) {
                        for (let l = 0; l < response.length; l++) {
                            if (this._baseProviderOptions.source === source_types_1.SourceType.MetaMask) {
                                this.saveJsonRpcLog(response[l], response[l].result.result, responseTimeMs[l]);
                            }
                            else {
                                this.saveJsonRpcLog(response[l], response[l].result, responseTimeMs[l]);
                            }
                        }
                    }
                    else {
                        if (this._baseProviderOptions.source === source_types_1.SourceType.MetaMask) {
                            ...
                            this.saveJsonRpcLog(response, response.result.result, responseTimeMs[0]);
                        }
                        else {
                            this.saveJsonRpcLog(response, response.result, responseTimeMs[0]);
                        }
                    }
                }
                else {
                    ...
                    if (Array.isArray(response)) {
                        for (let l = 0; l < response.length; l++) {
                            this.saveJsonRpcLog(response[l], errorMessage, responseTimeMs[l], true);
                        }
                    }
                    else {
                        this.saveJsonRpcLog(response, errorMessage, responseTimeMs[0], true);
                    }
                }
            }
            catch (error) {
    ...
    send(methodOrPayload, parameters = [], _fromSendAsync = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.web3Version === enums_2.Web3Versions.one &&
                this._isCustomProvider &&
                ...
                // any whitelisted providers which have `send`
                // with method and parameters and not the entire
                // payload
                switch (this._baseProviderOptions.source) {
                    case source_types_1.SourceType.ZeroXMesh:
                ...
                if (this.isUsingWeb3ProviderEngine() ||
                    (this._provider.send === undefined &&
                        this._provider.sendAsync !== undefined)) {
                    ...
                    // also if they dont have `send` defined and have `sendAsync` defined
                    // we should go through this path as well
                    return yield this.sendAsyncOnProviderPromiseWrapper(methodOrPayload);
                }
                else {
                    // if its not a defined source type then it should call
                    // send with as per Eip1193Provider
                    if (!Object.values(source_types_1.SourceType).includes(this._baseProviderOptions.source)) {
                        const result = yield this._provider.send(methodOrPayload.method, methodOrPayload.params);
                        return { result };
                    }
                    // @ts-ignore
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const netVersionJsonRpcPayload = {
                    id: 0,
                    jsonrpc: '2.0',
                    method: enums_1.JsonRpcMethods.net_version,
                    params: [],
                };
                if (this.web3Version === enums_2.Web3Versions.one &&
                    this._isCustomProvider &&
                    ...
                    switch (this._baseProviderOptions.source) {
                        ...
                        case source_types_1.SourceType.WalletLink:
                        case source_types_1.SourceType.Alchemy:
                            return this.parseNetworkVerion(yield provider.send(netVersionJsonRpcPayload.method, netVersionJsonRpcPayload.params));
                    }
                    // if they are using `web3-provider-engine` due to that behaviouring
                    // differently it needs to go into `sendAsync` and wait for the callback
                    if (this.isUsingWeb3ProviderEngine() ||
                        (provider.send === undefined &&
                            provider.sendAsync !== undefined)) {
                        return this.parseNetworkVerion(yield this.sendAsyncOnProviderPromiseWrapper(netVersionJsonRpcPayload));
                    }
                    // if its not a defined source type then it should call
                    // send with as per Eip1193Provider
                    if (!Object.values(source_types_1.SourceType).includes(this._baseProviderOptions.source)) {
                    ...
                    return this.parseNetworkVerion(yield provider.send(netVersionJsonRpcPayload));
                }
                const result = yield provider.send(netVersionJsonRpcPayload.method, netVersionJsonRpcPayload.params);
                return this.parseNetworkVerion(result);
            }
            catch (error) {
                // mute error so we can state unknown
            }
    ...
    saveBulkToLogs(bulkPayloads) {
        for (let i = 0; i < bulkPayloads.length; i++) {
            ...
            this.saveJsonRpcLog(info.payload, info.payload.result, undefined, info.isError);
        }
    }
    /**
     * Set the chain id
    ...
    sendAck(chainId) {
        return __awaiter(this, void 0, void 0, function* () {
            const ackRequest = {
                ...
                nodeUrl: this.getHost(),
                chainId,
                sourceCategory: Object(enums_2.SourceCategory)[this._baseOptions.source] || utils_1.Utils.UNKNOWN,
            };
     ...
     */
    parseNetworkVerion(networkVersion) {
        if (networkVersion) {
            if (typeof networkVersion === 'string') {
                return networkVersion.toString();
            }
            else {
                return networkVersion.result.toString();
            }
        }
        return utils_1.Utils.UNKNOWN;
    }
     ...
     * @param payload The JSONRPC payload
     */
    sendAsyncOnProviderPromiseWrapper(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield new Promise((resolve, reject) => {
                // @ts-ignore
                this._provider.sendAsync(payload, (error, result) => {
                    if (error) {
                        reject(error);
                    }
                    else {
     ...
     */
    getHost() {
        let host = utils_1.Utils.UNKNOWN;
        switch (this._providerType) {
            case enums_2.ProviderType.httpProvider:
                host = this.getHttpProviderHost();
                break;
            case enums_2.ProviderType.websocketProvider:
    ...
    getMetaMaskChainId() {
        if (this._baseProviderOptions.source === source_types_1.SourceType.MetaMask) {
            if (this._provider && this._provider.networkVersion) {
