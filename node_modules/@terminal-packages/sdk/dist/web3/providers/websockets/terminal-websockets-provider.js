...
const json_rpc_mapper_1 = __importDefault(require("../../../web3/mappers/json-rpc-mapper"));
...
class TerminalWebsocketProvider extends terminal_base_provider_1.TerminalBaseProvider {
    constructor(options) {
        ...
        if (options.customWebsocketProvider) {
            ...
            // if host is passed it set `this.host` to it
        ...
        if (this.web3Version === enums_1.Web3Versions.two) {
            ...
            if (_host && _host !== utils_1.Utils.UNKNOWN) {
     ...
     * @param _fromSendAsync This tells us if send async did the call which means `saveJsonRpcLog`
     *                       has already been called. web3 have different behaviour between 1.0
     *                       and 2.0 so have to handle some edge cases here.
     */
    send(method, parameters = [], _fromSendAsync = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.web3Version === enums_1.Web3Versions.two) {
                ...
                if (!_fromSendAsync) {
                    this.saveJsonRpcLog(payload, response.result, end - start);
                }
    ...
    on(event, callback) {
        ...
        this.websocketProvider.on(event, (result) => {
            if (this.web3Version === enums_1.Web3Versions.two) {
                ...
                this.saveJsonRpcLog(json_rpc_mapper_1.default.toPayload(subscription.subscribeMethod, subscription.parameters), result.result, undefined, event === 'error');
            }
            else {
                ...
                if (this._lastNodeSavedResult) {
                    if (this._lastNodeSavedResult.nodeResult ===
                        utils_1.Utils.parseJsonRPCResult(parsedResult)) {
                ...
                if (shouldSaveLogs) {
                    this._lastNodeSavedResult = {
                        nodeResult: utils_1.Utils.parseJsonRPCResult(parsedResult),
                        firedAtTimestamp: new Date().getTime(),
                    };
                    this.saveJsonRpcLog(json_rpc_mapper_1.default.toPayload(result.method, []), parsedResult, undefined, event === 'error');
                }
            }
            callback(result);
        });
